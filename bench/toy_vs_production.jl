...
#
# Note the toy runs roughly 150-250x slower (machine- and band-dependent), so
...
#     i.e. **190.8x** — a second run of the same command gave 177.1x, and the
#     Xeon Silver 4114 gave 200.4x (359 vs 1.79 us/trial).  That 177-200 span is
#     the scatter to read any single number against.
#   * the sigma table below came out DIGIT-FOR-DIGIT identical on both hosts,
#     which is the point of it: sigma is a property of the data and the
#     algorithm, so a host-dependent result there would mean a bug.
